...
export async function getCurrentUserProfile() {
  ...
  const { data: profile, error } = await supabase
    .from("profiles")
    .select("id, username, display_name, bio, school, program, avatar_url, avatar_position, banner_url, banner_position")
    .eq("id", user.id)
    .single();
...
export async function updateProfile(formData) {
  ...
  const { data: profile, error: fetchError } = await supabase
    .from("profiles")
    .select("id, avatar_url, avatar_position, banner_position")
    .eq("id", user.id)
    .single();
