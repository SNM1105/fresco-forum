...
export default async function Nav() {
  ...
  if (user) {
    const { data } = await supabase
      .from("profiles")
      .select("username, display_name, avatar_url, avatar_position, program")
      .eq("id", user.id)
      .single();
    profile = data;
  }
          ...
          {profile ? (
            <>
              <Link href={`/profile/${profile.username}`}>
                <Avatar name={profile.display_name || profile.username} url={profile.avatar_url} program={profile.program} position={profile.avatar_position} size={30} />
              </Link>
              <SignOutButton />
            </>
          ) : (
